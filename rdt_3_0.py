...
class RDT:
		...
		while True:
			...
			if timed_out == False:
				...
				if Packet.corrupt(self.byte_buffer[:length]): 
					...
					if response.msg_S == '1':
						print('***** Sender: Receiver ACK - Moving on to Next Packet *****') 
	...
	def rdt_3_0_receive(self):
		...
		while True:
			...
			# if byte_buffer is not shorter then Packet we have not received enough bytes
			if len(self.byte_buffer) < Packet.length_S_length:
